...
void del_last_user(struct user *ps);
//Заполение списка пользователей
struct user* init_users(struct user *usn);
int delete_user(struct user *ps, char *n, char *s);
...
/2(info name surname)/3(delete last user)/4(delete user)\n");
        scanf("%c%*c", &input);
        if(input == '0'){
            printf("Exit program\n");
            break;
            ...
            r1 = delete_user(users, name, surname);
            if(r1 == 1){
                printf("User not found\n");
            }
        }
...
int delete_user(struct user *ps, char *n, char *s){
    while (ps != NULL){
        if(ps->next != NULL){
            if(str_comp(n, ps->next->name) == 0 \
            && str_comp(s, ps->next->surname) == 0 ){
                struct user *us1 = ps->next->next;
                free(ps->next);
                ps->next = us1;
                printf("Delete user\n");
                return 0;
